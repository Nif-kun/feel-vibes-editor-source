...
func save_data(temp_dir:String, config_file:String, save_file:String, new_save:bool=true):
	...
	if new_save:
		...
		ShortLib.write_text_file(temp_dir+"/"+config_file, JSON.print(data, "\t"))
	Zipper.Zip(temp_dir, save_file, true, false)


func load_data(save_file:String, extract_path:String, config_file:String):
	Zipper.Unzip(save_file, extract_path)
	var texture_images = ShortLib.get_dir_image_textures(extract_path)
	var config_json = JSON.parse(ShortLib.read_text_file(extract_path+"/"+config_file)).result
